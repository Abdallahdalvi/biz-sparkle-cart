...
export function CookieBanner() {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem("Aghanims Phones and Gadgets-cookies-ok")) setShow(true);
  }, []);
  if (!show) return null;
  return (
    <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-6 md:max-w-md z-50 bg-white shopify-border shopify-shadow p-4 flex items-start gap-3">
      <p className="text-xs text-on-surface-variant leading-relaxed flex-1">
        ...
        Aghanims Phones and Gadgets you accept our{" "}
      ...
      <button
        onClick={() => {
          localStorage.setItem("Aghanims Phones and Gadgets-cookies-ok", "1");
          setShow(false);
        }}
